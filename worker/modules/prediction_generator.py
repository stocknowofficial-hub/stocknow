...
5. timeframe은 14 / 30 (일) 중 근거의 성격(단기 이슈 vs 구조적 변화)에 맞게 선택. 7일은 절대 사용 금지.
...
2. 타겟 구체화 및 다양성 (매우 중요): '비료 관련주', '방산 기업' 같은 두리뭉실한 범주형 단어는 절대 target으로 사용 금지. 뻔한 1등 대장주에만 얽매이지 말고, 트럼프 발언의 세부 뉘앙스(예: 무인기, 사이버 보안, 소형 원전, 셰일가스 등)에 가장 부합하는 구체적 종목을 매번 새롭게 발굴하여 선정할 것.
3. 코드 매칭: target_code에는 실제 미국 주식 티커 또는 한국 6자리 코드를 정확히 입력할 것. 가상의 코드를 지어내지 말 것.
...
JSON 배열만 출력 (마크다운 코드블록 ```json 등 절대 금지, 설명 없이 순수 JSON 텍스트만 출력):
[
  {{
    "prediction": "[트럼프] 발언 핵심 → 특정 종목명 단기 전망 (1줄)",
    "direction": "up 또는 down",
    "target": "정확한 상장 종목명 또는 ETF명",
